...
#define WindowName  "alternative hack for csgo"
...
void CMenu::Run()
{
	...
	ImGui::SetNextWindowSize(ImVec2(ImGui::GetIO().DisplaySize.x - 44.f, ImGui::GetIO().DisplaySize.y - 100.f), ImGuiCond_Once);
	...
	if (m_Page == PAGE_GLOBALS_AIMBOT)
	{
		...
		ImGui::SliderFloat("aiming downward offset", &vars::aimbot::downoffset, 0.f, 5.0f);
		ImGui::SliderFloat("aiming transverseoffset offset", &vars::aimbot::transverseoffset, -1.0f, 1.0f);
		const char* pcszBones[] = { "Head", "Neck", "CHEST", "STOMACH", "WAIST" };
		ImGui::Combo("Bone", &vars::aimbot::bone, pcszBones, IM_ARRAYSIZE(pcszBones));
		ImGui::Checkbox("Aiming on LSHIFT", &vars::aimbot::aiming_on_rmb);
	...
	else if (m_Page == PAGE_GLOBALS_VISUALS)
	{
		...
		{
			ImGui::ColorEdit3("##invis enemy", vars::visuals::col_enemy);
			ImGui::SameLine();
			ImGui::Text("enemy   ");
			ImGui::SameLine();
			ImGui::ColorEdit3("##invis teammate", vars::visuals::col_teammate);
			ImGui::SameLine();
			ImGui::Text("teammate    ");
			...
			const char* pcszStatusbarWay[] = { "Left", "Right", "Upper","Bottom" };
			ImGui::Combo("Statusbar way", &vars::visuals::statusbar_way, pcszStatusbarWay, IM_ARRAYSIZE(pcszStatusbarWay));
		...
		if (ImGui::TreeNode("ESP"))
		{
			...
			//ImGui::Checkbox("Only_one_Name", &vars::visuals::only_one_name);
			//ImGui::InputText("target name", vars::visuals::target_name, IM_ARRAYSIZE(vars::visuals::target_name));
	...
	ImGui::SameLine();
	ImGui::SetCursorPosX(ImGui::GetWindowSize().x - ImGui::CalcTextSize("by The_Itach1").x - 15.f);
	ImGui::Text("by The_Itach1");
	ImGui::PopFont();
